...
//			echo("Will apply move $algorithm[$i] $times times<br>");
	...
	function cofnij($k, $znak, $was)
	{
	 		$this -> kostka[$k+1] = $znak.'!!';
			
			if($was == 'e')
			{
				$roznica = 12-$k;			
				for($i = $k+2;$k<20;$k++)
						 $this -> kostka[$k] = $this -> kostka[$k+$roznica];
			}
			...
			if($was == 'c')
			{
				$roznica = 20-$k;
				for($i=0;$i<=$roznica;$i++)
				array_pop($this -> kostka);
			}	
		}
		
		
	function getElemsName($alg, $mode)
	{
			$alg = implode(' ', $this -> parse($alg));
			$numbers = $this ->process($alg, $mode);		
			$j=0;	
				for($i = 0;$i<count($numbers)-6;$i++)	
				{
					if($i<12)
					{
								if($numbers[$i] == $i)	$kostka[$i] = '!';
								
								else if($numbers[$i] == $i+12) $kostka[$i] = '-!';
								
								else
								$kostka[$i] = $this ->edge_names[$numbers[$i]];}	
					
					else 
					{
					 	      if($numbers[$i] == $j) $kostka[$i] = '!';
								 
								 else if($numbers[$i] == $j+8) $kostka[$i] = '+!';
								 
								 else if($numbers[$i] == $j+16) $kostka[$i] = '-!';
								 
								 else $kostka[$i] = $this ->corner_names[$numbers[$i]];
					$j++;
				}
				}
				
			//	foreach($kostka as $val) echo $val.'<br />';
						
					// sprawdzanie krawędzi (zamiana ! ! !.. -> !!)
					
				$i=11;
				$prawda = false;
				$znak = '';
				while($kostka[$i] == '!' && $i>=0) $i--; 
				

				if($i > 9) { $i=11;	while($kostka[$i] == '-!' && $i>=0) $i--; if($i<10) $prawda = true; $znak = '-';}
				else $prawda = true;
			...
			if($prawda)
			{
	          $kostka[$i+1] = $znak.'!!';							  
				...
				for($i = $i+2;$i<$ile;$i++)
						 $kostka[$i] = $kostka[$i+$roznica];
				
				$ile = count($kostka);
				
				for($i = $ile;$i>$ile-$roznica;$i--)		
					 array_pop($kostka);				
			}
			
				//to samo dla rogów (zamiana ! ! !.. -> !!)
			
			$rozmiar = count($kostka);
			$i=$rozmiar-1;
			$prawda = false;
			$znak = '';
			...
			while($kostka[$i] == '!' && $i>=0) $i--; 
				...
				if($i > $rozmiar-2)
				{ 
				  if($kostka[$rozmiar] == '-!'){$i=$rozmiar;	while($kostka[$i] == '-!' && $i>=0) $i--; if($i<$rozmiar - 2) $prawda = true; $znak = '-';}
				  if($kostka[$rozmiar] == '+!'){$i=$rozmiar;	while($kostka[$i] == '+!' && $i>=0) $i--; if($i<$rozmiar - 2) $prawda = true; $znak = '+';}
				}
				else $prawda = true;
